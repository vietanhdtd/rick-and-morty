...
function LocationsPage() {
  ...
  return <section className="archive-page"><header className="page-intro"><p className="eyebrow">Places / {query.data.info.count} total</p><h1>Explore the places that connect the cast.</h1><p>Planets, citadels, alternate dimensions, and places worth a closer look.</p></header><div className="location-grid">{query.data.results.map((location, index) => <article className="location-card" key={location.id} style={{ '--i': index } as React.CSSProperties}><div className="location-card__glyph"><MapPin /></div><span className="eyebrow">#{String(location.id).padStart(3, '0')} / {location.type || 'Unknown'}</span><h2>{location.name}</h2><p>{location.dimension}</p><footer><span>{location.residents.length} residents</span><Link to="/locations/$locationId" params={{ locationId: String(location.id) }} aria-label={`View ${location.name}`}><ArrowRight size={18} /></Link></footer></article>)}</div><div className="pagination"><button type="button" disabled={!query.data.info.prev} onClick={() => setPage((value) => value - 1)}>Previous</button><span>Page {page} / {query.data.info.pages}</span><button type="button" disabled={!query.data.info.next} onClick={() => setPage((value) => value + 1)}>Next</button></div></section>
}
