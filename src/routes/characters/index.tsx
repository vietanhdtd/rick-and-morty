...
function CharactersPage() {
  ...
  const query = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage) =>
      lastPage.info.next ? Number(new URL(lastPage.info.next).searchParams.get('page')) : undefined,
  })
  ...
  return (
    <section className="px-4 pt-12 sm:px-8 lg:px-16">
      <header className="mb-6 max-w-3xl">
        ...
        <h1 className="mb-4 text-[clamp(2.6rem,6vw,5.7rem)]">Find a character.</h1>
      ...
      <div className="mb-5 border border-border bg-surface-raised">
        <label className="flex items-center gap-3 p-4 text-signal">
          ...
          <input
            ...
            className="min-w-0 flex-1 border-0 bg-transparent text-xl tracking-[-.03em] text-content outline-none"
          />
        </label>
      ...
      {query.isPending ? (
        ...
        <>
          <div
            ...
            aria-live="polite"
          >
            <Filter size={14} aria-hidden="true" /> {query.data?.pages[0].info.count} matching
            characters
          </div>
          ...
          {query.hasNextPage && (
            ...
            >
              {query.isFetchingNextPage ? 'Loading more…' : 'Load more characters'}
            </button>
          )}
        </>
      )}
