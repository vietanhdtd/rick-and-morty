...
async function request<T>(path: string, schema: z.ZodType<T>): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      "We could not load the guide. Try again.",
      response.status,
    );
  }
...
): Promise<ApiPage<Character>>;
export function getPage(
  resource: "location",
  filters?: Record<string, string | number | undefined>,
): Promise<ApiPage<Location>>;
export function getPage(
  resource: "episode",
  filters?: Record<string, string | number | undefined>,
): Promise<ApiPage<Episode>>;
export function getPage(
  resource: ResourceKind,
  filters: Record<string, string | number | undefined> = {},
): Promise<ApiPage<Character> | ApiPage<Location> | ApiPage<Episode>> {
  ...
  ) as Promise<ApiPage<Character> | ApiPage<Location> | ApiPage<Episode>>;
}
...
export function getEntity(
  ...
  return request(
    `/${resource}/${id}`,
    schemas[resource] as z.ZodTypeAny,
  ) as Promise<Character | Location | Episode>;
}
...
export function getCharacters(ids: number[]) {
  ...
  return request(
    `/character/${ids.join(",")}`,
    z.union([characterSchema, z.array(characterSchema)]),
...
export function getEpisodes(ids: number[]) {
  ...
  return request(
    `/episode/${ids.join(",")}`,
    z.union([episodeSchema, z.array(episodeSchema)]),
  ).then((result) => (Array.isArray(result) ? result : [result]));
}
...
export async function getRandomSignal(
  ...
  const first =
    resource === "character"
      ? await getPage("character", { page: 1 })
      : resource === "location"
        ? await getPage("location", { page: 1 })
        : await getPage("episode", { page: 1 });
  const page = Math.floor(Math.random() * first.info.pages) + 1;
  const source =
    page === 1
      ? first
      : resource === "character"
        ? await getPage("character", { page })
        : resource === "location"
          ? await getPage("location", { page })
          : await getPage("episode", { page });
...
export const queryKeys = {
  entity: <T extends ResourceKind>(resource: T, id: string | number) =>
    [resource, id] as const,
  page: <T extends ResourceKind>(
    resource: T,
    filters: Record<string, unknown>,
  ) => [resource, "page", filters] as const,
  signal: (resource: ResourceKind, refresh: number) =>
    ["signal", resource, refresh] as const,
};
