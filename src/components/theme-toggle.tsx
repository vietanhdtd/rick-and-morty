...
function getCircularClipPaths(x: number, y: number) {
  ...
  return [`circle(0% at ${point})`, `circle(${(radius / referenceRadius) * 100}% at ${point})`]
}
...
export function ThemeToggle() {
  ...
  return (
    <button
      ref={buttonRef}
      className="inline-flex size-13 items-center justify-center border-l border-border text-content-muted transition-[background-color,color,transform] duration-150 hover:scale-105 hover:bg-signal-soft hover:text-signal active:scale-95"
      type="button"
      onClick={toggleTheme}
